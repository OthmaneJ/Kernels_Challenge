...
build_features_from_scratch = False
...
if __name__ == '__main__':
    ...
    if build_features_from_scratch:
        # Loading data
    ...
    sizes = [2, 3, 4, 5, 6, 7, 8] 
    predictions = [] # to store predictions from each classifier
    for size in tqdm(sizes):
        if build_features_from_scratch:
            # building features
            X,Y = get_features(sequences_train, sequences_test, size=size)

            if not os.path.exists(f'./features/size_{size}/'):
                os.makedirs(f'./features/size_{size}/')

            sparse.save_npz(f'./features/size_{size}/train_features.npz',X)
            sparse.save_npz(f'./features/size_{size}/test_features.npz',Y)
            
        else:

            X  = sparse.load_npz(f'./features/size_{size}/train_features.npz')
            Y  = sparse.load_npz(f'./features/size_{size}/test_features.npz')
